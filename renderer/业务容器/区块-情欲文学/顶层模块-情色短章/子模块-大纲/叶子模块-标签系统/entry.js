...
// ===== 标签芯片 =====
function 短watchAiSuggestions(fieldId, containerId) {
  ...
  var obs = new MutationObserver(function() {
    container.querySelectorAll('.ai-chip').forEach(function(chip) {
      chip.addEventListener('click', function() {
        ...
        if (ds) ds.innerHTML += 短renderTagChips([text]);
...
// ===== 标签芯片渲染 =====
function 短renderTagChips(tags, tagList) {
...
function 短ovAddTag(containerId, tag) {
  if (!tag) return;
  ...
  chips.innerHTML += 短renderTagChips([tag]);
}

// 暴露到全局
window.短watchAiSuggestions = 短watchAiSuggestions;
window.短renderTagChips = 短renderTagChips;
window.短ovAddTag = 短ovAddTag;

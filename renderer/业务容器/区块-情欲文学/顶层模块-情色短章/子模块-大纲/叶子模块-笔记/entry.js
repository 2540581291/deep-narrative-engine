function 短aiGenerateNotes() {
  // 已迁移至 registerAiField('gen_notes')，通过 openAiGenPanel 调用
  // 保留此函数以防旧按钮引用
  openAiGenPanel('V_gen_notes');
}

function 短renderOutlineNotes(el) {
  Store.vignette.get(短ot).then(function(m) {
    ...
    var h = '<div class="flex justify-between items-center mb-6">' +
      ...
      '<button class="btn" id="notesAiGenBtn" class="bg-accent fs-12" onclick="openAiGenPanel(\'V_gen_notes\')">🤖 AI 生成备注</button></div>' +
      '<div id="notesList">';
    ...
    document.getElementById('noteAddBtn').addEventListener('click', function() {
      Store.vignette.get(短ot).then(function(m2) {
        m2 = m2 || {};
        if (!m2.writingNotes) m2.writingNotes = [];
        m2.writingNotes.push('');
        Store.vignette.save(短ot, m2);
        短otab('notes');
      });
    });

// 小说大纲 · 新建作品
function 短renderOutlineCreate(el) {
  debugLog('novel', '短renderOutlineCreate START', '短ot=' + 短ot);
  // el 可能是外层容器（novelViewContent），也可能是 短outlineTabContent
  var contentEl = el || document.getElementById('短outlineTabContent');
  if (!contentEl) { debugLog('novel', '短renderOutlineCreate', '容器元素不存在！'); return; }
  var th = '';
  ...
  contentEl.innerHTML =
    ...
    '<div class="form-group"><label>章节数量</label><div class="flex-row"><input type="number" id="novelChapterCount" class="llm-input" style="width:100px" value="1" min="1" max="100">' +
    '<span class="text-muted text-sm">章（默认1章）</span></div></div>' +
  ...
  document.getElementById('novelCreateBtn').addEventListener('click', function() {
    ...
    var cc = Math.max(1, Math.min(100, parseInt(document.getElementById('novelChapterCount').value) || 1));
    document.getElementById('novelCreateStatus').textContent = 'AI 正在生成 ' + cc + ' 章大纲...';
    this.disabled = true;
    Store.vignette.save(title, { genreTags: tags, length: 'custom', premise: premise, chapterCount: cc, wordCount: 0, status: '规划中' }).then(function() {
      短ot = title; vignetteCurrentTitle = title;
      // 重新渲染大纲页面（带导航标签）
      渲染短章大纲(document.getElementById('novelViewContent'), title, { genreTags: tags, length: 'custom', premise: premise, chapterCount: cc });
    });
  });
}

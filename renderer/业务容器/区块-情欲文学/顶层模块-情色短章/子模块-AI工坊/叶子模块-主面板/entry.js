// AI 工坊工具函数
// 包含：渲染芯片组、构建写作指引、注入模板、预设管理、热力图、手动快照

// ===== AI 工坊工具函数 =====

// ===== 热力图和统计函数 =====

function 短updateKeystrokeDisplay() {
  var el = document.getElementById('writingKeystrokeCount');
  if (el) el.textContent = '⌨ ' + 短_writingKeystrokes + '键';
}

function 短updateSparkline() {
  var now = Date.now();
  if (now - 短_writingLastRecordTime < 2000) return;
  短_writingLastRecordTime = now;
  var editor = document.getElementById('writingEditor');
  if (!editor) return;
  短_writingKeystrokeLog.push(wordCount(editor.innerText));
  if (短_writingKeystrokeLog.length > 20) 短_writingKeystrokeLog.shift();
  短renderSparkline();
}

function 短renderSparkline() {
  var el = document.getElementById('writingSparkline');
  if (!el || !短_writingKeystrokeLog.length) return;
  var max = Math.max.apply(null, 短_writingKeystrokeLog) || 1;
  el.innerHTML = 短_writingKeystrokeLog.map(function(v) {
...
function 短手动快照() {
  ...
  var ch = 短写作章集[短写作当前章];
  var label = '第' + (短写作当前章 + 1) + '章 ' + (ch ? ch.title || '' : '');
  短_writingSnapshots.push({ time: Date.now(), label: label, content: editor.innerText });
  document.getElementById('writingSnapshotCount').textContent = '📸 ' + 短_writingSnapshots.length;
  toast('📸 快照已保存：' + label);
}

// ===== 连续性记录 =====

function 短loadStreakFromStorage() {
  try { var d = localStorage.getItem(短_writingStreakStoreKey); return d ? JSON.parse(d) : {}; } catch(e) { return {}; }
}

function 短recordWritingStreak() {
  try {
    var data = 短loadStreakFromStorage();
    var today = new Date().toISOString().slice(0,10);
    var editor = document.getElementById('writingEditor');
    var wc = editor ? wordCount(editor.innerText) : 0;
    if (wc > 0) { data[today] = (data[today] || 0) + 1; localStorage.setItem(短_writingStreakStoreKey, JSON.stringify(data)); }
  } catch(e) {}
}

function 短renderStreakBar() {
  ...
  for (var i = 6; i >= 0; i--) {
    ...
    days.push({ key: key, hasData: !!data[key], isToday: i === 0 });
  }
  el.innerHTML = '<div class="writing-streak-label">📅 本周写作</div><div class="writing-streak-days">' +
    days.map(function(d) {
      return '<div class="writing-streak-day' + (d.hasData ? ' on' : '') + (d.isToday ? ' today' : '') + '" title="' + d.key + (d.hasData ? ' ✓' : '') + '"></div>';
    }).join('') + '</div>';
}

// ===== 方案模式 =====
var 短_writingPlanStates = { A: false, B: false, C: false, D: false };
var 短_writingPlanResults = { A: '', B: '', C: '', D: '' };
var 短_writingPlanGeneration章 = 0;

var 短_writingPlanConfigs = {
...
function 短renderPlanChips() {
  ['A', 'B', 'C'].forEach(function(plan) {
    var cfg = 短_writingPlanConfigs[plan];
    var el = document.getElementById('plan' + plan + 'Chips');
    if (!el) return;
    el.style.display = 'grid';
    el.style.gridTemplateColumns = '1fr 1fr';
    el.style.gap = '3px 8px';
    el.innerHTML = 短_directionCategories.map(function(cat) {
      var chips = cat.options.map(function(o) {
        var active = cfg[cat.key] === o;
        return '<span class="writing-plan-chip' + (active ? ' active' : '') + '" onclick="短setPlanChip(\'' + plan + '\',\'' + cat.key + '\',\'' + o.replace(/'/g, "\\'") + '\')">' + o + '</span>';
      }).join('');
...
function 短setPlanChip(plan, field, value) {
  短_writingPlanConfigs[plan][field] = value;
  短renderPlanChips();
}

function 短toggleWritingPlan(plan) {
  短_writingPlanStates[plan] = !短_writingPlanStates[plan];
  var toggle = document.getElementById('toggle' + plan);
  var knob = document.getElementById('knob' + plan);
  if (toggle) toggle.style.background = 短_writingPlanStates[plan] ? '#4ecca3' : '#1e1e3a';
  if (knob) knob.style.left = 短_writingPlanStates[plan] ? '16px' : '2px';
}

// 存储生成前的原文，用于对比
var 短_writingOriginalText = '';

function 短writingGeneratePlans() {
  var enabledPlans = [];
  ['A', 'B', 'C', 'D'].forEach(function(plan) {
    if (短_writingPlanStates[plan]) enabledPlans.push(plan);
  });
  if (!enabledPlans.length) { toast('请先启用至少一个方案'); return; }
  // 记录生成时的章节索引，确保写入时写入正确的章节
  短_writingPlanGeneration章 = 短写作当前章;
  var ch = 短写作章集[短写作当前章];
  ...
  // 保存原文用于后续对比
  短_writingOriginalText = editorText;
  ...
  enabledPlans.forEach(function(plan) {
    ...
    var guidance = [];
    短_directionCategories.forEach(function(cat) {
      var val = cfg[cat.key];
      if (val && 短_directionMaps[cat.key] && 短_directionMaps[cat.key][val]) {
        guidance.push('· ' + 短_directionMaps[cat.key][val]);
      }
    });
    ...
    LLM.call({ prompt: userPrompt, system: sysPrompt, label: 'AI工坊·方案' + plan, temperature: 0.85 }).then(function(result) {
      ...
      短_writingPlanResults[plan] = result || '';
      _genCompleted++;
...
function 短writingPlanWrite(plan) {
  var result = 短_writingPlanResults[plan];
  if (!result) { toast('该方案还没有生成内容'); return; }

  // 切换到生成时的章节（如果不是当前章节），确保内容写入正确的章节
  if (短写作当前章 !== 短_writingPlanGeneration章) {
    短写作当前章 = 短_writingPlanGeneration章;
    短renderChapterList();
    var ch = 短写作章集[短写作当前章];
    var titleEl = document.getElementById('writingChapterTitle');
    if (titleEl) titleEl.textContent = ch ? (ch.title || '第' + (短写作当前章 + 1) + '章') : '';
  }

  var editor = document.getElementById('writingEditor');
  if (!editor) return;
  editor.innerHTML = 短formatForDisplay(result);
  短_writingPlanResults[plan] = '';
  var resultEl = document.getElementById('plan' + plan + 'Result');
  if (resultEl) resultEl.style.display = 'none';
  短updateWritingStatus();
  短章保存当前章();
  toast('方案 ' + plan + ' 内容已写入第' + (短写作当前章 + 1) + '章');
}

// window 导出
window.短toggleWritingPlan = 短toggleWritingPlan;
window.短setPlanChip = 短setPlanChip;
window.短writingGeneratePlans = 短writingGeneratePlans;
window.短writingPlanWrite = 短writingPlanWrite;
window.短renderPlanChips = 短renderPlanChips;
window.短loadStreakFromStorage = 短loadStreakFromStorage;
window.短recordWritingStreak = 短recordWritingStreak;
window.短renderStreakBar = 短renderStreakBar;
window.短updateKeystrokeDisplay = 短updateKeystrokeDisplay;
window.短updateSparkline = 短updateSparkline;

// ===== 对比功能 =====

// 将相邻的 add/remove 合并为 change（配对到同一行）
function 短mergeDiff(diff) {
  var merged = [];
  var i = 0;
  while (i < diff.length) {
    if (diff[i].type === 'same') {
...
function 短computeDiff(original, modified) {
  var re = /[^。！？\n]+[。！？\n]?/g;
  ...
  return 短mergeDiff(result);
}

function 短writingShowDiff(plan) {
  var result = 短_writingPlanResults[plan];
  var original = 短_writingOriginalText;
  if (!result) { toast('该方案还没有生成内容'); return; }
  if (!original) { toast('没有原文可对比'); return; }

  var diff = 短computeDiff(original, result);
...
window.短writingShowDiff = 短writingShowDiff;
window.短renderSparkline = 短renderSparkline;
window.短手动快照 = 短手动快照;

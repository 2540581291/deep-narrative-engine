...
var _ovAutoSaveTimer = null;
function saveDelayed() { if (_ovAutoSaveTimer) clearTimeout(_ovAutoSaveTimer); _ovAutoSaveTimer = setTimeout(小说大纲保存, 500); }
...
function 小说大纲保存() {
  Store.novel.get(_outlineTitle).then(function(m) {
    ...
    m.chapterCount = Math.max(1, Math.min(100, parseInt(document.getElementById('ovChapterCount').value) || 10));
    Store.novel.save(_outlineTitle, m);
  });
}

// 暴露到全局
window.saveDelayed = saveDelayed;
window.小说大纲保存 = 小说大纲保存;

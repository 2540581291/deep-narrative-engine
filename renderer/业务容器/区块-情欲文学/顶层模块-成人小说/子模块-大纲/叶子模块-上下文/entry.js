...
function getOutlineContext(title) {
  return Store.novel.get(title).then(function(m) {
    m = m || {};
...
function getOutlineContextWithoutChars(title) {
  return Store.novel.get(title).then(function(m) {
    m = m || {};
    var domGenre = [], domPlay = [];
    Array.from(document.querySelectorAll('#ovGenreChips .ov-tag-del')).forEach(function(el) {
...
window.formatCharContext = formatCharContext;

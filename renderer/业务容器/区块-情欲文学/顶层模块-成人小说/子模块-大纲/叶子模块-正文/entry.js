...
function renderOutlineMain(el) {
  ...
  if (!_outlineChapters.length) {
    Store.novel.get(_outlineTitle).then(function(data) {
      if (data && data.outline && data.outline.length) {
        _outlineChapters = data.outline;
        _outlineEnding = data.ending || '';
        ...
        // 默认展示一个空白章节
        _outlineChapters = [{index:1,title:'',playTags:'· ·',link:'',content:'',wordTarget:4000,eroticaLevel:'中度',highlight:false,characters:[],setting:''}];
      }
      renderOutlineList();
    });
    return;
...
function aiGenerateOutline(direction) {
  openAiGenPanel('gen_outline');
}
...
function renderOutlineList() {
  ...
  _outlineChapters.forEach(function(ch, i) { ch.index = i + 1; if (ch.characters && typeof ch.characters === 'string') ch.characters = ch.characters.split(/[、,，\s]+/).filter(Boolean); if (!ch.characters || !Array.isArray(ch.characters)) ch.characters = []; });
  ...
  var h = '<div class="flex justify-between items-center mb-6">' +
    ...
    '<div><button class="btn bg-accent fs-12" id="olRegenBtn" onclick="openAiGenPanel(\'gen_outline\')">🤖 AI 重绘大纲</button></div></div>' +
    '<!-- AI 选项容器 -->' +
    '<div id="aiopts_olChapTitle" class="dn mt-4"></div>' +
    '<div id="aiopts_olChapPlay" class="dn mt-4"></div>' +
    '<div id="aiopts_olChapLink" class="dn mt-4"></div>' +
    '<div id="aiopts_olChapContent" class="dn mt-4"></div>' +
    '<div id="aiopts_olChapChar" class="dn mt-4"></div>' +
    '<div id="aiopts_olChapSet" class="dn mt-4"></div>' +
    // ===== 大纲讨论区 =====
    ...
    '<button class="btn-sm bg-accent" onclick="genOutlineFromChat()" style="font-size:11px">🎯 生成大纲</button></div>' +
    '<div id="olChatMessages" style="height:280px;overflow-y:auto;padding:14px;background:var(--bg)">';
    if (_olChatMessages.length) {
      _olChatMessages.forEach(function(m) {
    ...
    '<textarea id="olChatInput" style="flex:1;min-height:38px;max-height:80px;resize:none;background:var(--bg);color:var(--fg);border:1px solid var(--border);border-radius:6px;padding:7px 12px;font-size:12px;font-family:inherit;outline:none" placeholder="输入你的想法..." onkeydown="if(event.keyCode===13&&!event.shiftKey){event.preventDefault();sendOutlineChatMsg()}"></textarea>' +
    '<button onclick="sendOutlineChatMsg()" style="height:36px;padding:0 18px;border-radius:6px;background:var(--accent);color:#fff;border:none;cursor:pointer;font-weight:600;font-size:12px;white-space:nowrap;font-family:inherit">发送</button>' +
    '</div></div></div>';
  _outlineChapters.forEach(function(ch, i) {
    var hc = ch.highlight;
    ...
    h += '  <div class="flex-row" style="justify-content:space-between;align-items:center;margin-bottom:8px">' +
         ...
         '<button class="ai-suggest-btn" onclick="_aiTargetId=\'olTitle_' + i + '\';openAiPanel(\'olChapTitle\')" title="AI 建议标题">🤖</button>' +
         '</div>' +
    ...
    h += '  <div class="flex-row" style="gap:8px;margin-bottom:6px">' +
         ...
         '<button class="ai-suggest-btn" onclick="_aiTargetId=\'olPlay_' + i + '\';_aiChapIdx=' + i + ';openAiPanel(\'olChapPlay\')">🤖</button></div></div>' +
         ...
         '<button class="ai-suggest-btn" onclick="_aiTargetId=\'olLink_' + i + '\';_aiChapIdx=' + i + ';openAiPanel(\'olChapLink\')">🤖</button></div></div></div>';
    // ===== 内容 =====
    h += '  <div class="form-group mb-6"><div class="text-sm text-muted">内容</div><div class="ai-field-row">' +
         ...
         '<button class="ai-suggest-btn" onclick="_aiTargetId=\'olContent_' + i + '\';_aiChapIdx=' + i + ';openAiPanel(\'olChapContent\')" style="align-self:stretch">🤖</button></div></div>';
    // ===== 字数 + 强度 + 角色 + 场景 =====
    h += '  <div class="flex-row" style="gap:8px;flex-wrap:wrap">' +
         ...
         '<button class="ai-suggest-btn" onclick="_aiTargetId=\'olChar_' + i + '\';_aiChapIdx=' + i + ';openAiPanel(\'olChapChar\')">🤖</button></div></div>' +
         ...
         '<button class="ai-suggest-btn" onclick="_aiTargetId=\'olSet_' + i + '\';_aiChapIdx=' + i + ';openAiPanel(\'olChapSet\')">🤖</button></div></div></div>';
    h += '</div>';
  });

  el.innerHTML = h;

  Array.from(el.querySelectorAll('.ol-del-ch')).forEach(function(btn) { btn.addEventListener('click', function() { var i = parseInt(this.getAttribute('data-oidx')); _outlineChapters.splice(i,1); renderOutlineList(); olAutoSave(); }); });
  Array.from(el.querySelectorAll('.ol-add-after')).forEach(function(btn) { btn.addEventListener('click', function() { var i = parseInt(this.getAttribute('data-oidx')); _outlineChapters.splice(i, 0, {index:i+1,title:'',playTags:'· ·',link:'',content:'',wordTarget:4000,eroticaLevel:'中度',highlight:false,characters:[],setting:''}); renderOutlineList(); olAutoSave(); }); });
  Array.from(el.querySelectorAll('.ol-regen-ch')).forEach(function(btn) { btn.addEventListener('click', function() { var i = parseInt(this.getAttribute('data-oidx')); _aiChapIdx = i; openAiGenPanel('olChapFull'); }); });
  Array.from(el.querySelectorAll('.ol-ch-title')).forEach(function(inp) { inp.addEventListener('change', function() { _outlineChapters[parseInt(this.getAttribute('data-oidx'))].title = this.value; olAutoSave(); }); });
  Array.from(el.querySelectorAll('.ol-hl-btn')).forEach(function(btn) {
    btn.addEventListener('click', function() {
      var i = parseInt(this.getAttribute('data-oidx'));
      var ch = _outlineChapters[i];
  ...
  ['ol-ch-play','ol-ch-link','ol-ch-char','ol-ch-set'].forEach(function(cls) {
    Array.from(el.querySelectorAll('.' + cls)).forEach(function(inp) {
      inp.addEventListener('change', function() {
        ...
        _outlineChapters[i][key === 'play' ? 'playTags' : key === 'char' ? 'characters' : key === 'set' ? 'setting' : key] = this.value;
        if (key === 'char') _outlineChapters[i].characters = this.value.split(/[、,，\s]+/).filter(Boolean);
        olAutoSave();
      });
    });
  });
  Array.from(el.querySelectorAll('.ol-ch-content')).forEach(function(ta) { ta.addEventListener('change', function() { _outlineChapters[parseInt(this.getAttribute('data-oidx'))].content = this.value; olAutoSave(); }); });
  Array.from(el.querySelectorAll('.ol-ch-wt')).forEach(function(inp) { inp.addEventListener('change', function() { _outlineChapters[parseInt(this.getAttribute('data-oidx'))].wordTarget = parseInt(this.value) || 4000; olAutoSave(); }); });
  Array.from(el.querySelectorAll('.ol-ch-ela')).forEach(function(sel) { sel.addEventListener('change', function() { _outlineChapters[parseInt(this.getAttribute('data-oidx'))].eroticaLevel = this.value; olAutoSave(); }); });
  Array.from(el.querySelectorAll('[draggable]')).forEach(function(item) { item.addEventListener('dragstart', function(e) { e.dataTransfer.setData('text/plain', this.getAttribute('data-oidx')); }); item.addEventListener('dragover', function(e) { e.preventDefault(); }); item.addEventListener('drop', function(e) { e.preventDefault(); var f = parseInt(e.dataTransfer.getData('text/plain')); var t = parseInt(this.getAttribute('data-oidx')); if (f !== t) { var m = _outlineChapters.splice(f,1)[0]; _outlineChapters.splice(t,0,m); renderOutlineList(); olAutoSave(); } }); });
}

// ===== 大纲讨论 - 聊天功能 =====
var _olChatMessages = [];

function renderOutlineChat() {
...
function sendOutlineChatMsg() {
  var input = document.getElementById('olChatInput');
  if (!input || !input.value.trim()) return;
  var msg = input.value.trim();
  input.value = '';
  _olChatMessages.push({ role: 'user', content: msg });

  // 每次调用都加载最新上下文拼入第一条 user 消息，确保 AI 始终知道完整大纲信息
  // LLM API 无状态，不能依赖 AI 记忆
  (typeof getOutlineContext === 'function' ? getOutlineContext(_outlineTitle) : Promise.resolve('')).then(function(ctx) {
    ctx = ctx || '';
    var msgs = [];
    _olChatMessages.forEach(function(m, idx) {
    ...
    LLM.call({ system: '你是一个情色小说大纲创作顾问。正在和作者讨论大纲设计。回复要简洁有建设性，给出具体建议。作者可以随时点击「从讨论生成大纲」来生成正式大纲。', messages: msgs, label: '大纲讨论' }).then(function(reply) {
      ...
      _olChatMessages.push({ role: 'assistant', content: reply });
      renderOutlineChat();
    }).catch(function(err) {
      if (typingEl) typingEl.style.display = 'none';
...
function genOutlineFromChat() {
  if (!_olChatMessages.length) { toast('请先和 AI 讨论大纲内容'); return; }
  if (!_outlineTitle) { toast('请先输入作品标题'); return; }
  // 使用与 AI 重绘大纲完全一致的提示词
  Store.novel.get(_outlineTitle).then(function(m) {
    ...
    return getOutlineContext(_outlineTitle).then(function(ctx) {
      ...
      // 取最后一次 AI 回复作为讨论内容
      var lastAi = '';
      for (var i = _olChatMessages.length - 1; i >= 0; i--) {
        if (_olChatMessages[i].role === 'assistant') { lastAi = _olChatMessages[i].content; break; }
      }
      ctx = ctx + '\n\n【AI 讨论内容】\n' + lastAi;
    ...
    LLM.callJSON({ label: '从讨论生成大纲', prompt: r.user, system: r.system, temperature: 0.7 }).then(function(d) {
      ...
      renderOutlineList();
      olAutoSave();
      toast('大纲已生成（共' + _outlineChapters.length + '章）');
    }).catch(function(err) {
      if (typingEl) typingEl.style.display = 'none';
...
window.sendOutlineChatMsg = sendOutlineChatMsg;
window.genOutlineFromChat = genOutlineFromChat;

var _outlineEnding = '';

// 即时保存：大纲正文的任何修改立即写入 Store
var _olSaveTimer = null;
function olAutoSave() {
  if (_olSaveTimer) clearTimeout(_olSaveTimer);
  _olSaveTimer = setTimeout(function() {
    if (!_outlineTitle || !_outlineChapters) return;
    Store.novel.get(_outlineTitle).then(function(m) {
      m = m || {};
      m.outline = _outlineChapters;
      m.ending = _outlineEnding;
...
function olAddCh() {
  _outlineChapters.push({index:_outlineChapters.length+1,title:'',playTags:'· ·',link:'',content:'',wordTarget:4000,eroticaLevel:'中度',highlight:false,characters:[],setting:''});
  renderOutlineList();
}
function olConfirm() { if (!_outlineChapters || !_outlineChapters.length) { toast('请先生成大纲'); return; } olDoConfirm(); }
function olDoConfirm() { Store.novel.get(_outlineTitle).then(function(m) { m = m || {}; m.outline = _outlineChapters; m.ending = _outlineEnding; m.status = '写作中'; Store.novel.save(_outlineTitle, m); novelCurrentTitle = _outlineTitle; toast('大纲已确认'); 小说切换视图('writing'); }); }

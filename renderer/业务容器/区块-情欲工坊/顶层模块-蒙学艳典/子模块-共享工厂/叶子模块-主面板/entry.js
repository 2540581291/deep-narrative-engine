// 情欲工坊 · 蒙学艳典 · 共享工厂
// 为 集部名篇/经部/史部/子部/兵书/笔记/医家/蒙学 提供统一的 list/editor/经典库 AI 逻辑（创作页与淫诗艳曲完全一致）
...
function 蒙学艳典工厂(cfg) {
  ...
  if (cfg.dbPath || cfg.dbDir) 导航.push({ id: 'classic', label: '📚 经典库' });
  var 当前视图 = 'list';
  var _editTitle = null;
  var 编辑状态 = {};
  var 经典分页 = 0;
  ...
  var 默认诗体 = cfg.默认诗体 || '无';
  var formOptions = cfg.formOptions || [];      // 蒙学模块不传（无诗体概念），默认空 → 不渲染诗体行
  ...
  function 渲染列表(el) {
    Store[cfg.storeKey].list().then(function(items) {
      ...
      if (筛选维度.indexOf('form') >= 0) {
        var formChips = ['全部'].concat(formOptions.indexOf('无') < 0 ? ['无'].concat(formOptions) : formOptions);
  ...
  function 渲染表单(el, data) {
    ...
    var formOpts = formOptions.indexOf('无') < 0 ? ['无'].concat(formOptions) : formOptions;
    var formOptionsHtml = formOpts.map(function(f) { return '<option value="' + f + '"' + (编辑状态.form === f ? ' selected' : '') + '>' + f + '</option>'; }).join('');
    var s = 编辑状态;
    ...
    h += '<select class="llm-input" id="' + cfg.prefix + 'Form" style="flex:1" onchange="' + cfg.windowPrefix + '编辑字段(\'form\',this.value);' + cfg.windowPrefix + '同步chips()">' + formOptionsHtml + '</select>';
    h += '</div></div>';
    // ② 选题卡：题材 / 露骨度 / 角色
    h += '<div class="n-card p-10 mb-6">';
    h += 共享卡片头('🎯', '选题', 'AI 可一键建议');
  ...
  function 经典筛选逻辑(all) {
    var filtered = all;
    if (经典筛选.book) filtered = filtered.filter(function(p) { return (p.book || '') === 经典筛选.book; });
  ...
  function 经典筛选栏HTML() {
    ...
    // 书目行（dbDir 模式：书作为筛选维度，单界面浏览）
    if (cfg.dbDir && 经典书目 && 经典书目.length) {
      h += '<div style="display:flex;align-items:center;gap:4px;margin-bottom:8px;flex-wrap:wrap">';
      h += '<span style="font-size:11px;color:var(--fg3);font-weight:500;width:32px;flex-shrink:0">书目</span>';
      h += chip(!f.book, '经典筛选书目(\'\')', '全部');
      经典书目.forEach(function(b) {
        h += chip(f.book === b.name, '经典筛选书目(\'' + b.name + '\')', escHtml(b.name) + (b.count ? '(' + b.count + ')' : ''));
      });
      h += '</div>';
    }
  ...
  // ===== 经典库（dbPath 读单文件 / dbDir 合并多文件为单列表，书目作为筛选维度）=====
  var 经典书目 = null; // dbDir 模式书目 [{ name, count }]

  // dbDir 模式：一次性加载目录下全部书，合并为单列表（每篇带 book 标记），书目作为筛选维度
  function 渲染经典库(el) {
    if (cfg.dbDir) {
      if (经典缓存) { 渲染经典列表(el); return; }
      el.innerHTML = '<div class="text-muted p-20 text-center">正在加载经典库...</div>';
      LocalFS.dbList(cfg.dbDir).then(function(entries) {
        entries = (entries || []).filter(function(e) { return !e.isDir && /\.json$/.test(e.name); });
        经典书目 = entries.map(function(e) { return { name: e.name.replace(/\.json$/, ''), count: 0 }; });
        if (!经典书目.length) { el.innerHTML = '<div class="placeholder-text">经典库暂无数据</div>'; return; }
        Promise.all(经典书目.map(function(b) {
          return LocalFS.dbRead(cfg.dbDir + '/' + b.name + '.json').then(function(text) {
            var items = text ? JSON.parse(text) : [];
            b.count = items.length;
            return items.map(function(p) {
              return { title: p.篇 || p.title || '无题', dynasty: '', author: '', content: p.原文 || p.content || '', source: '', cipai: '', book: b.name };
            });
          }).catch(function() { return []; });
        })).then(function(batches) {
          经典缓存 = { all: [].concat.apply([], batches) };
          渲染经典列表(el);
        });
      }).catch(function() { el.innerHTML = '<div class="placeholder-text">书目加载失败</div>'; });
      return;
    }
  ...
  function 经典筛选书目(b) { 经典筛选.book = b; 经典分页 = 0; 经典刷新(); }
  ...
  if (typeof registerAiField !== 'undefined') {
    ...
    registerAiField(cfg.aiFieldId, cfg.aiLabel, function() {
      ...
      var promptName = acrosticText ? 'poetry_acrostic_gen' : (cfg.promptName || 'meng_xue_gen');
  ...
  window[cfg.windowPrefix + '经典筛选书目'] = 经典筛选书目;

  return { 切换视图: 切换视图, 当前视图: function(){ return 当前视图; } };
}
window.蒙学艳典工厂 = 蒙学艳典工厂;

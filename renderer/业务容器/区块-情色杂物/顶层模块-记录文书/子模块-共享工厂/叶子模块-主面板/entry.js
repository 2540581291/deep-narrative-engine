// 情色杂物 · 记录文书 · 文书模块共享工厂
// 完全照抄 淫诗体模块工厂 的行式与内容：选题卡（题材/露骨度/角色）、素材卡（意象库/随机灵感）与淫诗一致
// 差异：诗体→文书类型（下拉）、无价位、无藏头输入
// 数据模型：{ title, type, genre[], explicit, content, imagery[], rhyme, appreciation, roles[], adaptSource/adaptExplicit/adaptLen }
...
function 记录文书工厂(cfg) {
  // cfg: { storeKey, containerId, viewContentId, prefix, windowPrefix, navLabelList, navLabelEdit, 文书类型选项, promptName, aiFieldId, aiLabel, 默认露骨度, 显示标签, 筛选维度, 意象库 }
  // 题材/露骨度/意象/角色 与淫诗艳曲完全一致；无价位、无藏头
  var 共享题材选项 = cfg.题材选项 || ['口交', '乳交', '调教', '绿帽', '人妻', '乱伦', '足交', '露出', '肛交', '群交'];
  var 共享露骨度选项 = cfg.露骨度选项 || ['含蓄隐晦', '粗俗露骨'];
  var 意象库 = cfg.意象库 || 记录文书意象库;   // 灵感素材卡数据源（默认内置全量意象库）
  var 导航 = [
  ...
  var 默认文书类型 = cfg.默认文书类型 || (cfg.文书类型选项 && cfg.文书类型选项[0]) || '';
  var 图标 = cfg.图标 || '📜';
  var 筛选状态 = {};        // 列表筛选：{ type:'全部', genre:'全部', explicit:'全部' }
  var 筛选维度 = cfg.筛选维度 || ['type'];
  ...
  function 渲染列表(el) {
    Store[cfg.storeKey].list().then(function(items) {
      var h = '';
      // 新建按钮：独立一行
      h += '<div class="mb-10"><button class="btn-new" onclick="' + cfg.windowPrefix + '新创作()">＋ 新建</button></div>';
      // 文书类型筛选行（默认第一维，对应淫诗 诗体）——与新建按钮分开放
      if (筛选维度.indexOf('type') >= 0) {
        h += 筛选行(cfg.文书类型标签 || '类型', ['全部'].concat(cfg.文书类型选项), 筛选状态.type, cfg.windowPrefix + '筛选', 'type');
      }
      ...
      var filtered = items || [];
      if (筛选状态.type && 筛选状态.type !== '全部') {
        filtered = filtered.filter(function(i) { return (i.type || 默认文书类型) === 筛选状态.type; });
      }
      ...
      if (!filtered.length) { h += '<div class="placeholder-text">暂无文书</div>'; }
      else {
        filtered.forEach(function(item) {
          ...
          var ty = item.type || 默认文书类型;
          if (ty && ty !== 默认文书类型) h += '<span class="badge-tag">' + escHtml(ty) + '</span>';
          ...
          h += '</div>';
          if (item.content) h += '<div class="text-muted text-sm mt-4" style="white-space:pre-wrap">' + escHtml(item.content).slice(0, 120) + '</div>';
  ...
  function 阅读(title) {
    Store[cfg.storeKey].get(title).then(function(item) {
      item = item || {};
      if (!item.content && !item.title) { toast('该文书暂无内容'); return; }
      ...
      var ty = item.type || 默认文书类型;
      if (ty && ty !== 默认文书类型) h += '<span class="reader-tag">' + escHtml(ty) + '</span>';
      ...
      h += '<div class="reader-poem-author">' + (ty && ty !== 默认文书类型 ? escHtml(ty) : '') + '<span class="reader-dot">·</span>' + escHtml(item.explicit || '') + '</div>';
      h += '<div class="reader-poem-text">' + escHtml(item.content || '暂无正文') + '</div>';
      h += '</div>';
      // 底部按钮栏
  ...
  function 复制全文(title) {
    Store[cfg.storeKey].get(title).then(function(item) {
      item = item || {};
      var text = (item.title || title) + (item.type ? ' · ' + item.type : '') + (item.explicit ? ' · ' + item.explicit : '') + '\n' + (item.content || '');
  ...
  function 自动建档() {
    ...
    return Store[cfg.storeKey].list().then(function(items) {
      for (var i = 0; i < items.length; i++) {
        if (items[i].title === t) { toast('同名文书已存在，草稿未建立'); return false; }
      }
      _editTitle = t;
      var data = {
        title: t, content: 编辑状态.content || '', tags: 编辑状态.tags || [],
        type: 编辑状态.type || 默认文书类型, explicit: 编辑状态.explicit || 默认露骨度,
  ...
  function 渲染编辑器(el) {
    if (_editTitle) {
      ...
      渲染表单(el, { title:'', content:'', tags:[], type:默认文书类型, genre:[], explicit:默认露骨度, imagery:[], rhyme:'', appreciation:'',
  ...
  // ===== 卡片式创作页（与淫诗艳曲完全一致：作品信息 / 选题 / 灵感素材 / AI 生成 / 改编；无藏头、无价位）=====
  ...
  function 渲染表单(el, data) {
    编辑状态 = { title:data.title||'', type:data.type||默认文书类型, genre:data.genre||[], explicit:data.explicit||默认露骨度, content:data.content||'', tags:(data.tags||[]).join('、'), imagery:data.imagery||[], rhyme:data.rhyme||'', appreciation:data.appreciation||'', roles:data.roles||[] };
    ...
    // ① 作品信息卡：标题（+AI 建议）| 文书类型下拉
    h += '<div class="n-card p-10 mb-6">';
    h += 共享卡片头('📋', '文书信息', '输入即自动保存');
    h += '<div class="ai-field-row">';
    h += '<input class="llm-input" id="' + cfg.prefix + 'Title" placeholder="文书名（失焦即自动建档）" value="' + escHtml(s.title||'') + '" style="flex:2" onchange="' + cfg.windowPrefix + '编辑字段(\'title\',this.value)">';
    h += '<button class="ai-suggest-btn" onclick="openAiGenPanel(\'' + cfg.aiFieldId + 'Suggest\')" title="AI 文书名建议">🤖</button>';
    h += '<select class="llm-input" id="' + cfg.prefix + 'Type" style="flex:1" onchange="' + cfg.windowPrefix + '编辑字段(\'type\',this.value);' + cfg.windowPrefix + '同步chips()">' + cfg.文书类型选项.map(function(c){ return '<option value="' + c + '"' + (s.type === c ? ' selected' : '') + '>' + c + '</option>'; }).join('') + '</select>';
    h += '</div></div>';
    // ② 选题卡：题材 / 露骨度 / 角色（与淫诗完全一致）
    h += '<div class="n-card p-10 mb-6">';
    h += 共享卡片头('🎯', '选题', 'AI 可一键建议');
    ...
    h += '<div class="mb-6">';
    h += '<textarea class="llm-input llm-input-lg" id="' + cfg.prefix + 'Direction" placeholder="主题 / 方向（可选，如：主仆关系明确、条款细致）" style="width:100%;height:64px;resize:vertical"></textarea>';
    h += '</div>';
  ...
  // ===== AI 提示词组装（角色段 / 参数段，与淫诗完全一致；无藏头、无价位）=====
  ...
  // 参数段：选题 + 灵感素材，有值才列；露骨度附解释
  function 共享参数上下文() {
    var ctx = '';
    if (编辑状态.type) ctx += '文书类型：' + 编辑状态.type + '\n';
  ...
  if (typeof registerAiField !== 'undefined') {
    ...
    registerAiField(cfg.aiFieldId, cfg.aiLabel, function() {
      ...
      var typeEl = document.getElementById(cfg.prefix + 'Type');
      if (typeEl) typeEl.value = s.type || 默认文书类型;
      ...
      var next = function() {
        if (_editTitle) {
          ...
          var data = {
            ...
            type: s.type || 默认文书类型, explicit: s.explicit || 默认露骨度,
    ...
    registerAiField(cfg.aiFieldId + 'Adapt', cfg.aiLabel + '改编', function() {
      ...
      // 改编专用参数段：含文书类型与题材，但不含露骨度（改编露骨度由改编卡「维持原度/污秽淫化」决定，避免两个露骨度指令冲突）
      var ctx = '';
      if (s.type) ctx += '文书类型：' + s.type + '\n';
      if (s.genre && s.genre.length) ctx += '题材：' + s.genre.join('、') + '\n';
      if (s.imagery && s.imagery.length) ctx += '素材：' + s.imagery.join('、') + '\n';
      var r = renderPrompt(cfg.promptName + '_adapt', {
        ctx: ctx,
        charCtx: 共享角色上下文(),
        source: source,
